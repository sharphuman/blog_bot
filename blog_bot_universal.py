...
# Designed to blend seamlessly into a Black/Dark Blue website.

FONT_STACK = "font-family: 'Inter', system-ui, -apple-system, sans-serif;"

STYLE_CONFIG = {
    # 1. Main Container: Transparent background so it fits your site.
    "WRAPPER": f"{FONT_STACK} max-width: 740px; margin: 0 auto; background: transparent; padding: 20px;",
    
    # 2. Typography: White/Grey text for Dark Mode readability
    ...
    "P": f"{FONT_STACK} color: #cbd5e1; font-size: 19px; line-height: 1.8; margin-bottom: 28px;", # #cbd5e1 is "Slate 300" (Soft White)
...
# This makes the preview box black so you can actually see the white text
st.markdown("""
<style>
...
if not st.session_state.blog_content:
    ...
    with col2:
        ...
        # We wrap the preview in a black box so you can see the white text in Streamlit
        st.markdown(f"""
        ...
        st.info("Copy this code. It will be WHITE text on a TRANSPARENT background.")
